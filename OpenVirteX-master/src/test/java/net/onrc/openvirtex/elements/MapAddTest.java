...
public class MapAddTest extends TestCase {
    ...
    public void testAddSwitches() {
        ...
        for (int i = 0; i < MapAddTest.MAXPSW; i++) {
            ...
            sw = new PhysicalSwitch(DatapathId.of(i), OFFactories.getFactory(OFVersion.OF_13));
            pSw.add(sw);
            for (int j = 0; j < MapAddTest.MAXTIDS; j++) {
            	//yk
            	//vsw = new OVXSingleSwitch(i, j);
            	vsw = new OVXSingleSwitch(DatapathId.of(i),  OFFactories.getFactory(OFVersion.OF_13), j);
                vSw.add(vsw);
    ...
    protected void makeSwitches(Map<Long, PhysicalSwitch> pswmap,
        ...
        for (long i = 0; i < MapAddTest.MAXTIDS / 2; i++) {
        	...
        	psw = new PhysicalSwitch(DatapathId.of(i), OFFactories.getFactory(OFVersion.OF_13)); /* DPID */
        ...
        for (long j = 0; j < MapAddTest.MAXTIDS / 2; j++) {
        	...
        	vsw = new OVXSingleSwitch(DatapathId.of(j), OFFactories.getFactory(OFVersion.OF_13), 1); /* DPID, TID */
            vswmap.add(vsw);
        }
