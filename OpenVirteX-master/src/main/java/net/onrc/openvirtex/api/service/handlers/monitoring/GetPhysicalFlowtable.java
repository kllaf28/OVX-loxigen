...
public class GetPhysicalFlowtable extends ApiHandler<Map<String, Object>> {
    ...
    public JSONRPC2Response process(final Map<String, Object> params) {
        try {
        	...
        	final DatapathId dpid = HandlerUtils.<DatapathId>fetchField(
                    MonitoringHandler.DPID, params, false, DatapathId.of(-1));
            ...
            if (dpid.getLong() == -1) {
                ...
                flows = aggregateFlowsBySwitch(dpid, map);
    ...
    private LinkedList<OVXFlowStatisticsReply> aggregateFlowsBySwitch(
            DatapathId datapathId, Mappable map) {
        LinkedList<OVXFlowStatisticsReply> flows = new LinkedList<OVXFlowStatisticsReply>();
        final PhysicalSwitch sw = PhysicalNetwork.getInstance().getSwitch(datapathId);
