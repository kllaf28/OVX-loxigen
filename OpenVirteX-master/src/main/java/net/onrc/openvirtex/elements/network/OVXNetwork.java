...
public class OVXNetwork extends Network<OVXSwitch, OVXPort, OVXLink> implements
    ...
    public void unregister() {
        ...
        final LinkedList<DatapathId> dpids = new LinkedList<>();
    ...
    public OVXSwitch createSwitch(final List<DatapathId> dpids, final DatapathId switchId)
            throws IndexOutOfBoundException {
        ...
        if (dpids.size() == 1) {
        	...
        	virtualSwitch = new OVXSingleSwitch(switchId,  OFFactories.getFactory(OFVersion.OF_13), this.tenantId);
        } else {
            //yk
        	//virtualSwitch = new OVXBigSwitch(switchId, this.tenantId);
        	virtualSwitch = new OVXBigSwitch(switchId, OFFactories.getFactory(OFVersion.OF_13), this.tenantId);
        }
    ...
    public OVXSwitch createSwitch(final List<DatapathId> dpids)
            throws IndexOutOfBoundException {
        //yk
    	//final long switchId = (long) 0xa42305 << 32
        //        | this.dpidCounter.getNewIndex();
        final DatapathId switchId = DatapathId.of((long) 0xa42305 << 32
                | this.dpidCounter.getNewIndex());
    ...
    public OVXPort createPort(final DatapathId physicalDpid, final short portNumber,
    ...
    public RoutingAlgorithms setOVXBigSwitchRouting(final DatapathId dpid,
    ...
    public Host connectHost(final DatapathId ovxDpid, final short ovxPort,
    ...
    */
    public synchronized OVXLink connectLink(final DatapathId ovxSrcDpid,
            final short ovxSrcPort, final DatapathId ovxDstDpid,
            final short ovxDstPort, final String alg, final byte numBackups,
            final int linkId) throws IndexOutOfBoundException,
            PortMappingException {
    ...
    */
    public synchronized SwitchRoute connectRoute(final DatapathId ovxDpid,
            final short ovxSrcPort, final short ovxDstPort,
            final List<PhysicalLink> physicalLinks, final byte priority,
            final int... routeId) throws IndexOutOfBoundException {
    ...
    public synchronized void removeSwitch(final DatapathId ovxDpid) {
        this.dpidCounter.releaseIndex((int) (0x000000 << 32 | (int)ovxDpid.getLong()));
    ...
    public synchronized void removePort(final DatapathId ovxDpid, final short ovxPort) {
        OVXPort port = this.getSwitch(ovxDpid).getPort(ovxPort);
        port.unregister();
    }
    ...
    public synchronized void disconnectRoute(final long ovxDpid,
            final int routeId) {
    */
    public synchronized void disconnectRoute(final DatapathId ovxDpid,
            final int routeId) {
        OVXBigSwitch sw = (OVXBigSwitch) this.getSwitch(ovxDpid);
    ...
    public synchronized void startSwitch(final DatapathId ovxDpid) {
        OVXSwitch sw = this.getSwitch(ovxDpid);
        sw.boot();
    }
    ...
    public synchronized void startPort(final DatapathId ovxDpid, final short ovxPort) {
        OVXPort port = this.getSwitch(ovxDpid).getPort(ovxPort);
        port.boot();
    }
    ...
    public synchronized void stopSwitch(final DatapathId ovxDpid) {
        OVXSwitch sw = this.getSwitch(ovxDpid);
        sw.tearDown();
    }
    ...
    public synchronized void stopPort(final DatapathId ovxDpid, final short ovxPort) {
        OVXPort port = this.getSwitch(ovxDpid).getPort(ovxPort);
        port.tearDown();
    }
